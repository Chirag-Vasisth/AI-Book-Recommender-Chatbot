...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function greetUser() {
        ...
        // Don't add a bot message here immediately because we have the static welcome screen
        ...
        fetch(`${API_BASE_URL}/api/chat`, {
            ...
            .then(data => {
                ...
                if (data.error) {
                    ...
                    addBotMessage("I apologize, but I'm unable to process that request right now.", messageId);
                } else {
                    ...
                    setTimeout(() => {
                        ...
                        const thanksLabel = feedbackContainer.querySelector('.feedback-thanks');
                        if (thanksLabel) {
                            thanksLabel.style.display = 'none';
                        }

                        const feedbackLabel = feedbackContainer.querySelector('.feedback-label');
                        if (feedbackLabel) {
    ...
    function parseBookRecommendations(message) {
        ...
        formattedMessage = formattedMessage.replace(/•/g, '<span style="color: var(--accent-secondary); font-weight: bold; margin-right: 8px;">•</span>');
        
        // Find and highlight book titles in quotes matching the expected "Title" by Author format
        const bookPattern = /"([^"]+)" by ([^\.,<]+)/g;
        return formattedMessage.replace(bookPattern, '<div class="book-suggestion"><span class="book-title">$1</span> by <span class="book-author">$2</span></div>');
    }
    ...
    document.querySelectorAll('.feedback-btn').forEach(btn => {
        btn.addEventListener('click', function () {
            ...
            if (currentFeedbackMessageId !== null) {
                ...
                // Show thanks and hide buttons
                this.parentElement.style.display = 'none';

                let thanksLabel = feedbackContainer.querySelector('.feedback-thanks');
                if (!thanksLabel) {
                    thanksLabel = document.createElement('span');
                    thanksLabel.className = 'feedback-thanks';
                    thanksLabel.style.display = 'block';
                    feedbackContainer.appendChild(thanksLabel);
                }

                thanksLabel.textContent = isLike ? 'Thanks! We\'re glad you liked the recommendation ✨' : 'Thanks for the feedback! We\'ll try to improve. 🛠️';
                feedbackContainer.querySelector('.feedback-label').style.display = 'none';
            }
        });
    });
    ...
    // Load any saved conversation (commented out to start fresh on refresh)
    // loadConversation();
});